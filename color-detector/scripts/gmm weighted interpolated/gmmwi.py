...
Now includes weighted average matching using cluster covariance for more robust color matching.
"""
...
MAX_K = 15 # Max components to test with BIC
TOP_N = 5  # Number of top matches to display
SUBSAMPLE_SIZE = 10000 # Max pixels per image for clustering speed
INTERPOLATION_RESOLUTION = 5 # Interpolation grid resolution
USE_INTERPOLATION = True
MATCHING_METHOD = 'covariance_weighted'  # Options: 'centroid', 'covariance_weighted', 'mahalanobis'
COVARIANCE_SAMPLES = 1000  # Number of samples for covariance-weighted matching
COVARIANCE_SCALE = 1.0  # Scale factor for covariance (1.0 = actual covariance)
# =================================
...
def load_munsell_groundtruth(csv_path, use_interpolation=USE_INTERPOLATION):
    ...
    if has_xyY and 'Y' in df.columns:
...
def cluster_bone_pixels_optimized(bone_lab, img_name, output_dir):
    """Clustering with GMM using BIC-optimized number of components, now returns covariances."""
    ...
    centers = gmm.means_
    covariances = gmm.covariances_  # Get the covariance matrices
    
    cluster_sizes = np.bincount(labels, minlength=optimal_k)
    
    return centers, cluster_sizes, labels, optimal_k, covariances
...
def find_top_matches_enhanced(cluster_center, gt_df, top_n=TOP_N):
    ...
    for idx in top_indices:
        ...
        results.append({
            'dE': dE[idx],
            'H': row.get('H_GT', row.get('H', None)),
            'V': row.get('V_GT', row.get('V', None)),
            'C': row.get('C_GT', row.get('C', None)),
            'L_gt': row['L_gt'],
            'a_gt': row['a_gt'],
            'b_gt': row['b_gt'],
...
def find_weighted_matches(cluster_center, cluster_covariance, gt_df, top_n=TOP_N, n_samples=COVARIANCE_SAMPLES, scale=COVARIANCE_SCALE):
    """
    Find the best Munsell matches using the entire cluster distribution.
    Samples points from the cluster distribution and calculates weighted average distance.
    """
    # Scale the covariance
    scaled_covariance = cluster_covariance * scale
    
    # Sample points from the cluster distribution
    try:
        sampled_points = np.random.multivariate_normal(
            cluster_center, scaled_covariance, n_samples
        )
    except:
        # If covariance matrix is not positive definite, use a diagonal approximation
        diag_cov = np.diag(np.diag(cluster_covariance)) * scale
        sampled_points = np.random.multivariate_normal(
            cluster_center, diag_cov, n_samples
        )
    
    gt_lab_arr = gt_df[['L_gt','a_gt','b_gt']].values.astype(float)
    
    # Calculate distances from all sampled points to all Munsell chips
    all_distances = []
    for point in sampled_points:
        dE = colour.delta_E(point[np.newaxis, :], gt_lab_arr, method='CIE 2000')
        all_distances.append(dE)
    
    all_distances = np.array(all_distances)
    
    # Calculate weighted average distance for each Munsell chip
    weighted_dE = np.mean(all_distances, axis=0)
    
    # Get indices of top N matches (lowest weighted dE)
    top_indices = np.argsort(weighted_dE)[:top_n]
...
def find_mahalanobis_matches(cluster_center, cluster_covariance, gt_df, top_n=TOP_N):
    """
    Find the best Munsell matches using Mahalanobis distance.
    This measures how many standard deviations away a Munsell chip is from the cluster center.
    """
    gt_lab_arr = gt_df[['L_gt','a_gt','b_gt']].values.astype(float)
    
    # Calculate Mahalanobis distance for each Munsell chip
    try:
        inv_cov = np.linalg.inv(cluster_covariance)
    except:
        # If covariance matrix is singular, use a diagonal approximation
        inv_cov = np.linalg.inv(np.diag(np.diag(cluster_covariance)))
    
    mahalanobis_dists = []
    
    for munsell_point in gt_lab_arr:
        diff = munsell_point - cluster_center
        mahalanobis_dist = np.sqrt(diff.T @ inv_cov @ diff)
        mahalanobis_dists.append(mahalanobis_dist)
    
    mahalanobis_dists = np.array(mahalanobis_dists)
    
    # Get indices of top N matches (lowest Mahalanobis distance)
    top_indices = np.argsort(mahalanobis_dists)[:top_n]
    ...
    for idx in top_indices:
        ...
        # Also calculate the regular ΔE for reference
        dE = colour.delta_E(cluster_center[np.newaxis, :], gt_lab_arr[idx:idx+1], method='CIE 2000')[0]
        
        results.append({
            'dE': dE,
            'mahalanobis': mahalanobis_dists[idx],
            'H': row.get('H_GT', row.get('H', None)),
            'V': row.get('V_GT', row.get('V', None)),
            'C': row.get('C_GT', row.get('C', None)),
            'L_gt': row['L_gt'],
...
def plot_cluster_distribution(bone_pixels_cluster, cluster_center, cluster_covariance, munsell_chip_lab, output_path):
    """Enhanced 3D scatter plot for the cluster with covariance ellipsoid and inferno colormap."""
    fig = plt.figure(figsize=(12, 10))
    ax = fig.add_subplot(111, projection='3d')
    
    # Subsample for visualization if too many points
    if len(bone_pixels_cluster) > 2000:
        indices = np.random.choice(len(bone_pixels_cluster), 2000, replace=False)
    ...
    # Plot cluster points with color based on distance
    ...
    ax.scatter(cluster_center[0], cluster_center[1], cluster_center[2], 
               c='red', marker='o', s=100, label='Cluster Center', edgecolors='black')
    
    # Plot best Munsell chip
    ax.scatter(munsell_chip_lab[0], munsell_chip_lab[1], munsell_chip_lab[2], 
               c='blue', marker='*', s=300, label='Best Munsell Chip', edgecolors='black')
    
    # Plot covariance ellipsoid (95% confidence interval)
    try:
        # Get eigenvalues and eigenvectors for the covariance matrix
        eigenvalues, eigenvectors = np.linalg.eigh(cluster_covariance)
        
        # Scale eigenvalues to get 95% confidence ellipsoid
        scale = np.sqrt(7.815)  # Chi-square value for 95% confidence with 2 degrees of freedom
        ...
        # Rotate and translate ellipsoid to correct position
        for i in range(len(x)):
            for j in range(len(x)):
                [x[i,j], y[i,j], z[i,j]] = np.dot([x[i,j], y[i,j], z[i,j]], eigenvectors) + cluster_center
                
        # Plot the ellipsoid
        ax.plot_wireframe(x, y, z, color='red', alpha=0.2, label='Covariance (95% CI)')
    except:
        print("Could not plot covariance ellipsoid - matrix may be singular")
    ...
    #plt.title('Cluster Distribution with Covariance Ellipsoid\n(Colored by Distance - Inferno Colormap)')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()

def create_enhanced_visualization(img_path, cluster_centers, cluster_sizes, cluster_covariances, top_matches_list, 
    ...
    for i, cluster_idx in enumerate(top_indices):
        ...
        method = match.get('method', 'centroid')
    ...
    # 2. 3D scatter plot for best cluster with covariance
    ...
    scatter_path = os.path.join(output_dir, f"{img_name}_3d_scatter_covariance.png")
    plot_cluster_distribution(best_cluster_pixels, cluster_centers[best_cluster_idx], 
                             cluster_covariances[best_cluster_idx], best_munsell_lab, scatter_path)
...
def process_single_image(img_path, gt_df, output_dir):
    ...
    try:
        ...
        # Apply optimized clustering (now GMM) - updated to return covariances
        cluster_centers, cluster_sizes, cluster_labels, optimal_k, cluster_covariances = cluster_bone_pixels_optimized(
        ...
        for i, (center, covariance) in enumerate(zip(cluster_centers, cluster_covariances)):
            if MATCHING_METHOD == 'covariance_weighted':
                top_matches = find_weighted_matches(center, covariance, gt_df, top_n=TOP_N)
            elif MATCHING_METHOD == 'mahalanobis':
                top_matches = find_mahalanobis_matches(center, covariance, gt_df, top_n=TOP_N)
            else:
                # Default to centroid-based matching
            ...
            for j, match in enumerate(top_matches):
                cluster_results.append({
                    ...
                    "matching_method": match.get('method', 'centroid'),
                    "mahalanobis_dist": match.get('mahalanobis', None)
                })
        
        # Create enhanced visualizations
        create_enhanced_visualization(img_path, cluster_centers, cluster_sizes, cluster_covariances, top_matches_list,
                                    optimal_k, bone_lab, cluster_labels, gt_df, output_dir, img_name)
...
def main():
    ...
    if MATCHING_METHOD == 'covariance_weighted':
        print(f"Covariance samples: {COVARIANCE_SAMPLES}, Scale: {COVARIANCE_SCALE}")
    ...
    if all_results:
        ...
        # Method comparison
        if 'matching_method' in res_df.columns:
            method_stats = res_df.groupby('matching_method')['dE2000'].agg(['mean', 'std', 'min', 'max', 'count']).round(3)
            method_csv = os.path.join(OUTPUT_DIR, "method_comparison.csv")
            method_stats.to_csv(method_csv)
            print(f"Method comparison: {method_csv}")
